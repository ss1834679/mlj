import { formatTime } from '../../utils/im/index'
...
const conversationModules = {
  ...
  getters: {
    ...
    totalUnreadCount: state => {
      ...
      if (result === 0) {
        console.log("没有新消息", 'totalUnreadCount');
        // todo 使用一个变量保存，然后在列表中去一一对应展示
        // wx.removeTabBarBadge({ index: 0 })
      } else {
        console.log("有新消息", state.allConversation, 'totalUnreadCount');
        // todo 使用一个变量保存，然后在列表中去一一对应展示
  ...
  actions: {
    ...
    getMessageList(context) {
      ...
      if (!context.state.isCompleted) {
        ...
        if (!context.state.isLoading) {
          ...
          uni.$app.getMessageList({ conversationID: currentConversationID, nextReqMessageID: nextReqMessageID, count: 15 }).then(res => {
    ...
    checkoutConversation(context, conversationID) {
      ...
      return uni.$app.getConversationProfile(conversationID)
        .then(({ data: { conversation } }) => {
          console.log(conversation, "conversation拿到的聊天记录？？");
          ...
          console.log("// todo 去对话页面");
          uni.navigateTo({ url: `/pagesC/chat/chat?toAccount=${name}&type=${conversation.type}` })
